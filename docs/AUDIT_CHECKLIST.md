...
| ID | Requirement | Source | Status |
|---|---|---|---|
| S1 | The chain is self-perpetuating: each clear is fed back and terminates itself while enabling the next gate; each gate is fed back, terminates itself, and enables the next clear. Order within a cycle: (Red or Black clear) → (Green or Yellow gate) → (Yellow or Green clear) → (Black or Red gate). Its outputs are four clear-selector voltages (R¹, R₁, R², R₂) and four gate driver-driver voltages for RI and RII; only RI's clears and gates are fed back. | p. 142, p. 146–152 | OK (per-cycle order); see S1a |
| S1a | At the end of the second half all three toggles are back at 1 and, with Counter Stop still high, the next Red or Black clear follows at once with no external stimulus. With Counter Stop low the supertoggle cannot enter its 1 condition (grid 6 held below cut-off), so the chain goes quiescent at the cycle boundary. Counter Stop is a level, not a pulse; it both initiates and terminates the process. | p. 142–146, p. 152 | DIFF (finding 9: one-shot model) |
| S2 | Lags: ≈1.5 µs between initiation and termination of a clear; a gate is disabled ≈1.5 µs after it is enabled. So a half cycle ≈ 3 µs and a plain shift cycle ≈ 6 µs; a Record half-cycle additionally waits the Carry Delay (N8). The 1.5 µs figure is stated only for the Black Clear and Yellow Gate hops; applying it to the other six hops is an assumption. | p. 148, p. 150 | OK |
| S2a | The chassis-1 and chassis-5 toggles are cleared by the same Black and Red clear buses as the register toggles ("the voltage of the Black Clear bus falling from +150v to +50v. Evidently this also clears the toggle in Chassis 1"), and a clear needs the bus below +70 v for ≈1 µs. So in the Record branch the chassis-5 toggle flips and the Red Clear terminates ≈1.5 µs after initiation, like every other clear; the Carry Delay must sit between clear termination and Green Gate enabling. The report does not locate the Carry Delay Unit (chassis Del, Dwg. A-1455) in the chain; the April figure of ≈15 µs from initiation to Green Gate is the only magnitude. | p. 54, p. 147–148, p. 152; chassis list; Apr. 1954 p. I-31 | DIFF (finding 11) |
| S3 | Red vs Black clear (Accept vs Reject) at the start of every cycle is chosen by the Accept-Reject Selector (Dwg. O-1463) "for all orders": from the content of 2⁻³⁹R₂ in multiplication, from the trial result in division. It must be sampled live each cycle. Yellow vs Green clear (right vs left) at the second half is chosen by the LR chassis feeding the R₁ Clear Selector (Fig. 12: R from LR-A2, L from LR-A8): set by the multiply/divide toggle for × and ÷, but alternating left, right for the additive orders (A2, A3). | p. 73, p. 147, p. 151, Fig. 12 | DIFF (finding 2: digit 0 still takes Adder path) |
...
| S6 | Shift Counter: scaling type, six stages (2⁰..2⁵), two ranks True and False. Prepared by clearing T to 000000 and F to 01111 (2⁰ = 0, others 1). Counts once per RI cycle: the input is a pair of pulses E₀′ then E₀″ per item, supplied from RI's own clear/gate signals (Fig. VI "STEP", "a signal from RI advances the Shift Counter"). The 2⁵ stage is simplified: the counter reads 1..47 correctly and returns to 32 on the 48th count. Fig. 30 gives the full T and F contents for counts 0..40. | p. 127–136, p. 171, p. 173, Fig. 29, Fig. 30, Fig. VI | SIMPLIFIED (finding 9) |
